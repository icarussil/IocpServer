...
// pnOutErrCode  : 위와 상동
BOOL IocpHandler::Asscoicate(SOCKET hSocket, ULONG_PTR CompletionKey, int* pnOutErrCode)
{
	assert(hSocket != INVALID_SOCKET);

	return Asscoicate((HANDLE)hSocket, CompletionKey, pnOutErrCode);
}
...
// CompletionKey : 연결되면서 쓰여질 Completion Key
// pnOutErrCode  : 위와 상동
BOOL IocpHandler::Asscoicate(HANDLE hDevice, ULONG_PTR CompletionKey, int* pnOutErrCode)
{
...
BOOL IocpHandler::PostCompletionStatus(ULONG_PTR CompletionKey, DWORD dwBytesTransferred, OVERLAPPED* pOverlapped, int* pnOutErrCode)
{
	...
	if ( FALSE == ret && NULL != pnOutErrCode)
	{
		*pnOutErrCode = GetLastError();
	}
